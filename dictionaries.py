...
"""
Create a function iterateDictionary2(key_name, some_list) that, given a list of dictionaries and a key name, the function prints the value
stored in that key for each dictionary. For example, iterateDictionary2('first_name', students) should output:
...
def iterateDictionary2(students, key_name):
    for i in range (len(students)):
...
def printInfo(dojo, key_name):
        length = len(dojo[key_name])
        print(str(length) + ' ' + (key_name))
        print(dojo[key_name])
